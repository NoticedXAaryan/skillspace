...
export function successStandard(message: string, fields: Record<string, string> = {}): void {
  ...
  console.log();
  const lines = Object.entries(fields).map(([k, v]) => c.textMuted(padLabel(k)) + c.text(v));
  if (lines.length > 0) {
...
export function successCritical(
  headline: string,
  body: string,
  hints: [string, string][] = [],
): void {
  ...
  hints.forEach(([label, value]) => {
    console.log('  ' + c.brand(CHARS.ARROW) + '  ' + c.textMuted(padLabel(label)) + c.info(value));
  });
  console.log();
}

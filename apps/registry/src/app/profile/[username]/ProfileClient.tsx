...
export default function ProfileClient({ profile }: { profile: ProfileData }) {
  const [activeTab, setActiveTab] = useState<
    'skills' | 'collections' | 'followers' | 'achievements'
  >('skills');
  ...
  return (
    <div className="min-h-screen bg-black pb-24">
      ...
      <div className="relative">
        ...
        >
          {!profile.banner && (
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom,rgba(34,211,238,0.15),transparent_70%)]" />
          )}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-2">
            {profile.github && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1.5 text-sm text-neutral-400 hover:text-cyan-400 transition-colors"
              >
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="flex items-center gap-6 border-b border-white/10 mb-8 overflow-x-auto no-scrollbar">
          ...
          >
            ...
            Skills{' '}
            <span className="bg-neutral-800 text-neutral-300 px-1.5 py-0.5 rounded text-xs ml-1">
              {profile.stats.packages}
            </span>
          </button>
          <button
        ...
        <div>
                      ...
                      <CardContent className="p-6">
                        ...
                        <div className="flex items-center gap-4 text-xs text-neutral-500 font-mono">
                          <span className="flex items-center gap-1.5">
                            <Star className="w-4 h-4 text-amber-500" /> {pkg._count?.stars || 0}
                          </span>
                        </div>
                      </CardContent>
                    </Card>
          ...
          {activeTab === 'collections' && (
            <div className="py-20 text-center text-neutral-500 border border-dashed border-white/10 rounded-xl bg-neutral-950/50">

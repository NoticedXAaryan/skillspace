    ...
    <main className="min-h-screen bg-black pb-24">
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="flex gap-2 mb-8 border-b border-white/10 pb-4 overflow-x-auto">
          <Button
            variant="secondary"
            className="bg-white/10 text-white hover:bg-white/20 border border-white/10 rounded-full h-9"
          >
            All Requests
          </Button>
          <Button
            variant="ghost"
            className="text-neutral-400 hover:text-white hover:bg-white/5 rounded-full h-9"
          >
            Open Bounties
          </Button>
          <Button
        ...
        {requests.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {requests.map((req) => (
              <Card
                key={req.id}
                className="bg-neutral-950 border-white/10 hover:border-cyan-500/30 transition-colors group overflow-hidden"
              >
                <CardContent className="p-0 flex flex-col sm:flex-row">
                  ...
                  <div className="bg-black/50 p-6 sm:w-64 border-t sm:border-t-0 sm:border-l border-white/10 flex flex-row sm:flex-col items-center justify-between gap-4">
                    <div className="flex flex-col">
                      <span className="text-xs text-neutral-500 uppercase tracking-wider font-semibold mb-1">
                        Bounty
                      </span>

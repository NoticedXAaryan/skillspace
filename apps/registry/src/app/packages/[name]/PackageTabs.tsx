...
import { Star, MessageSquare, BookOpen, BarChart3, TrendingUp, Clock } from 'lucide-react';
...
export default function PackageTabs({
  ...
  const [activeTab, setActiveTab] = useState<'readme' | 'reviews' | 'discussions' | 'analytics'>(
    'readme',
  );
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      ...
      <div className="p-8">
        {activeTab === 'readme' && (
          <div className="prose prose-invert max-w-none text-muted-foreground">{readmeContent}</div>
        )}
          ...
          <div>
            ...
            <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="rounded-lg border border-border bg-card p-6 shadow-sm">
                <div className="mb-2 flex items-center gap-2 text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                  <TrendingUp className="h-3.5 w-3.5" /> Downloads
                </div>
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Install trend charts will appear once usage data is collected.
              </p>
            </div>
          </div>
        )}

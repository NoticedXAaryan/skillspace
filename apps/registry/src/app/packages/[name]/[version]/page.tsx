    ...
    if (line.trim().startsWith('```')) {
      if (inCodeBlock) {
        elements.push(
          <div
            key={`code-${i}`}
            className="my-6 overflow-x-auto rounded-md bg-zinc-950 p-4 font-mono text-sm shadow-sm"
          >
            <pre className="m-0 text-zinc-300">
    ...
    <main className="container mx-auto px-4 py-12 pb-24">
      {!isLatest && (
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-md border border-amber-500/30 bg-amber-500/10 p-4">
          ...
          <Link
            href={`/packages/${name}`}
            className="inline-flex h-9 items-center justify-center rounded-md bg-secondary px-4 text-xs font-medium text-secondary-foreground transition-colors hover:bg-secondary/80"
          >
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <h1 className="mb-2 text-4xl font-extrabold tracking-tight text-foreground">
                  {pkg.name}{' '}
                  <span className="text-2xl font-normal text-muted-foreground">v{version}</span>
                </h1>
              ...
              {readme ? (
                ...
                <>
                  ...
                  <div className="rounded-md bg-zinc-950 p-4 font-mono text-sm text-zinc-300 shadow-sm">
                    <code>
                      skillspace install {pkg.name}@{version}
                    </code>
                  </div>
                  ...
                  <div className="rounded-md bg-zinc-950 p-4 font-mono text-sm text-zinc-300 shadow-sm">
                    <code>
                      skillspace run {pkg.name}@{version} --input ./src
                    </code>
                  </div>
                </>
              )}
                ...
                <div>
                  ...
                  <div className="font-semibold text-foreground">
                    {targetVersion?.publishedAt
                      ? new Date(targetVersion.publishedAt).toLocaleDateString()
                      : 'N/A'}
                  </div>
                </div>
              </div>

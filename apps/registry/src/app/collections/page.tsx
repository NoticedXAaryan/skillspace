...
export default function CollectionsPage() {
  ...
  const grouped = examplesData.reduce(
    (acc, ex) => {
        ...
        {Object.keys(grouped).map((category) => (
          <div key={category} className="flex flex-col gap-6">
            <div className="flex justify-between items-end border-b border-white/10 pb-4">
              ...
              <Link
                href={`/examples?category=${category}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {grouped[category].slice(0, 3).map((ex: any) => (
                <Card
                  key={ex.id}
                  className="bg-neutral-950 border-white/10 hover:border-cyan-500/50 hover:shadow-[0_8px_32px_rgba(34,211,238,0.15)] transition-all duration-300 flex flex-col justify-between group"
                >
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex gap-3 text-xs text-neutral-400">
                        <span className="flex items-center gap-1">
                          <Download className="w-3 h-3" /> {ex.downloads.toLocaleString()}
                        </span>
                        <span className="flex items-center gap-1 text-yellow-500/80">
                          <Star className="w-3 h-3" /> {ex.stars.toLocaleString()}
                        </span>
                      </div>
                    </div>
                    <Link

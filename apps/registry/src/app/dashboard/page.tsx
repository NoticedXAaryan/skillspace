  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {[
          { label: 'Total Packages', value: totalPackages, icon: Package, color: 'text-blue-400', bg: 'bg-blue-400/10' },
          { label: 'Total Downloads', value: totalDownloads.toLocaleString(), icon: Download, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
          { label: 'GitHub Linked', value: user.packages.filter(p => p.githubUrl).length, icon: TrendingUp, color: 'text-purple-400', bg: 'bg-purple-400/10' },
          { label: 'Account', value: user.plan, icon: Users, color: 'text-amber-400', bg: 'bg-amber-400/10' },
        ].map((stat) => (
          <div key={stat.label} className="group relative rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl p-6 overflow-hidden hover:border-white/20 transition-all duration-300">
            <div className={`absolute top-0 right-0 w-32 h-32 rounded-full ${stat.bg} blur-3xl -mr-16 -mt-16 transition-transform group-hover:scale-150 duration-500`} />
            ...
            <p className="relative z-10 text-4xl font-bold text-white tracking-tight">{stat.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        ...
        >
          ...
          <h3 className="relative z-10 text-lg text-white font-bold mb-2 group-hover:text-blue-400 transition-colors">My Packages</h3>
        ...
        >
          ...
          <h3 className="relative z-10 text-lg text-white font-bold mb-2 group-hover:text-purple-400 transition-colors">Playground</h3>
      ...
      <div className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/[0.02]">
          ...
          <a href="/dashboard/packages" className="text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">View all →</a>
        </div>
        {user.packages.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <a href="/create" className="inline-flex items-center text-sm font-medium text-blue-400 hover:text-blue-300">
          ...
          <div className="divide-y divide-white/5">
            {user.packages.map((pkg) => (
              <div key={pkg.id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-6 hover:bg-white/[0.02] transition-colors">

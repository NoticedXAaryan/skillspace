...
const Node = ({
  ...
  <div
    className={cn(
      'flex flex-col items-center justify-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900/50 p-6 text-sm font-medium shadow-sm backdrop-blur-sm transition-colors hover:border-zinc-700',
      className,
    )}
  >
...
const Connector = ({ horizontal = true }: { horizontal?: boolean }) => (
  <div
    className={cn(
      ...
      horizontal ? 'h-0.5 w-16' : 'h-16 w-0.5',
    )}
  >
    <motion.div
      className={cn(
        'absolute bg-primary',
        horizontal ? 'inset-y-0 h-full w-1/2' : 'inset-x-0 h-1/2 w-full',
      )}
      ...
      <div className="flex w-full items-center justify-center py-12">
        ...
        >
          ...
          <Node
            ...
            className="border-primary/50 [&>svg]:text-primary"
          />
        </motion.div>
      </div>
    );

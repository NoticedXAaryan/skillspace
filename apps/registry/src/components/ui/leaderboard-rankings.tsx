...
export const LeaderboardRankings = React.forwardRef<HTMLDivElement, LeaderboardRankingsProps>(
  (
    { className, rankings, currentUserId, showPagination = false, defaultPageSize = 10, ...props },
    ref,
  ) => {
        ...
        {paginatedRankings.map((ranking) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-sm font-bold font-mono">{formatValue(ranking.value)}</div>
            </div>
          );
        })}
        ...
        {showPagination && totalPages > 1 && (
          <div className="flex items-center justify-between mt-4 border-t border-border pt-4">
            <span className="text-xs text-muted-foreground">
              Showing {(page - 1) * defaultPageSize + 1} to{' '}
              {Math.min(page * defaultPageSize, rankings.length)} of {rankings.length}
            </span>

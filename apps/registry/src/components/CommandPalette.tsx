      ...
      <CommandList>
        ...
        {!query && (
          <CommandGroup heading="Trending Searches">
            <CommandItem
              onSelect={() => {
                router.push('/search?q=summarize');
                setOpen(false);
              }}
            >
          ...
          <CommandGroup heading="Popular Tags">
            <div className="flex flex-wrap gap-2 p-2">
              ...
              <Badge
                ...
                className="cursor-pointer"
                onClick={() => {
                  router.push('/search?q=computer-vision');
                  setOpen(false);
                }}
              >
                computer-vision
              ...
              <Badge
                ...
                className="cursor-pointer"
                onClick={() => {
                  router.push('/search?q=data-processing');
                  setOpen(false);
                }}
              >
                data-processing
        ...
        {(query.toLowerCase().includes('install') ||
          query.toLowerCase().includes('run') ||
          query.toLowerCase().includes('publish') ||
          query.toLowerCase().startsWith('skillspace')) && (
          <CommandGroup heading="CLI Commands (Click to copy)">
            <CommandItem onSelect={() => handleCopyCommand('skillspace install @skillspace/agent')}>
              {copiedCmd === 'skillspace install @skillspace/agent' ? (
                <CheckCircle2 className="mr-2 h-4 w-4 text-green-500" />
              ) : (
                <Terminal className="mr-2 h-4 w-4 text-primary" />
              )}
              <span>
                skillspace install <span className="text-muted-foreground">@skillspace/agent</span>
              </span>
              <Copy className="ml-auto h-3 w-3 opacity-50" />
            </CommandItem>
            <CommandItem onSelect={() => handleCopyCommand('skillspace run @skillspace/summary')}>
              {copiedCmd === 'skillspace run @skillspace/summary' ? (
                <CheckCircle2 className="mr-2 h-4 w-4 text-green-500" />
              ) : (
                <Terminal className="mr-2 h-4 w-4 text-primary" />
              )}
              <span>
                skillspace run <span className="text-muted-foreground">@skillspace/summary</span>
              </span>
              <Copy className="ml-auto h-3 w-3 opacity-50" />
            </CommandItem>
            <CommandItem onSelect={() => handleCopyCommand('skillspace publish')}>
              {copiedCmd === 'skillspace publish' ? (
                <CheckCircle2 className="mr-2 h-4 w-4 text-green-500" />
              ) : (
                <Terminal className="mr-2 h-4 w-4 text-primary" />
              )}
